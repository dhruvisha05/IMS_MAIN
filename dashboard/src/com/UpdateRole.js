...
function Updaterole() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3000/role/role_update/${id}`, {
            headers: {
                authorization: token
            }
        })
    ...
    const updaterole = (e) => {
        e.preventDefault();
        axios.post(`http://localhost:3000/role/role_update/${id}`, {
            name: name,
        }, {
            headers: {
                authorization: token
            ...
            <Container fluid className="ps-0">
                <Row className="gx-0">
                    ...
                    <Col className='d-flex justify-content-center'> 
                        <div>
                            <form method='post' className='my-5' onSubmit={updaterole}>
                                <table border={1} className='border-1 border-black p-3 rounded-2'>
                                    <tr className='my-3 text-center'>
                                        <th colSpan={2} className='h1 py-3'>UPDATE ROLE</th>  
                                    </tr>
                                    <tr className='my-3'>
